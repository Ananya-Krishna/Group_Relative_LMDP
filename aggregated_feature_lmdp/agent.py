...
def eps_greedy_adaptive(mdp, lam, num_epi, init_eps, final_eps):
    ...
    for k in range(num_epi):
        if eps > final_eps + (init_eps - final_eps)/ num_epi: 
...
def compute_optimal_policy(mdp):
    """
    Compute the optimal policy using dynamic programming.
    Returns: opt_policy[h, s] = best action at timestep h in state s
    """
    V = np.zeros((mdp.horizon + 1, mdp.num_state))  # V_H = 0 by definition
    opt_policy = np.zeros((mdp.horizon, mdp.num_state), dtype=int)

    for h in reversed(range(mdp.horizon)):
        for s in range(mdp.num_state):
            q_vals = []
            for a in range(mdp.num_action):
                r = mdp.reward_func[h, s, a]
                p = mdp.tran_func[h, s, a, :]
                q_vals.append(r + np.dot(p, V[h + 1]))
            V[h, s] = np.max(q_vals)
            opt_policy[h, s] = np.argmax(q_vals)
...
def optimal_agent(mdp, num_epi):
    """
    Executes the optimal policy computed from known transition and reward functions.
    """
    opt_policy = compute_optimal_policy(mdp)
    ...
    return accu_reward[1:], episode_reward, episode_accu_reward[1:]
...
def lsvi_ucb(mdp, lam , num_epi, p, c, set_beta):
    ...
    for k in range(num_epi):
        ...
        for h in range(horizon - 1, -1, -1):
            ...
            fst_term = np.einsum('d,sad->sa', mat_w[h, :], mdp.phi[h, :, :, :])
            lambda_inv = np.linalg.pinv(mat_lambda[h, :, :], rcond=1e-10)
