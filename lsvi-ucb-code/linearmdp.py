...
class linMDP(object):
    ...
    def generate_linearMDP(self):
        ...
        # 🛠 Fix negative values
        self.tran_func = np.clip(self.tran_func, a_min=0, a_max=None)

        # 🛠 Normalize properly
        for h in range(self.horizon):
            for s in range(self.num_state):
                for a in range(self.num_action):
                    total = self.tran_func[h, s, a, :].sum()
                    if total == 0:
                        # If sum is zero, assign uniform distribution
                        self.tran_func[h, s, a, :] = 1.0 / self.num_state
                    else:
                        # Else normalize
                        self.tran_func[h, s, a, :] /= total
...
from helper import standard_feature_map, group_invariant_feature_map
import numpy as np

def create_mdp(grid_size, horizon, feature_type="standard"):
    """
    Create a Linear MDP with standard or group-invariant feature maps.
    
    Args:
        grid_size (int): Gridworld size (e.g., 4 for 4x4 grid).
        horizon (int): Number of timesteps H.
        feature_type (str): "standard" or "group".
    
    Returns:
        mdp: an instance of linMDP with phi constructed.
    """
    mdp = linMDP(grid_size, horizon=horizon)  # or however linMDP is initialized
    num_states = mdp.num_state
    num_actions = mdp.num_action

    # Determine feature dimension
    if feature_type == "standard":
        dim = num_states * num_actions
    elif feature_type == "group":
        max_offset = grid_size - 1
        dim = (2 * max_offset + 1)**2 * num_actions
    else:
        raise ValueError("Unknown feature type")

    phi = np.zeros((horizon, num_states, num_actions, dim))

    for h in range(horizon):
        for s in range(num_states):
            for a in range(num_actions):
                if feature_type == "standard":
                    phi[h, s, a, :] = standard_feature_map(s, a, num_states, num_actions)
                elif feature_type == "group":
                    phi[h, s, a, :] = group_invariant_feature_map(s, a, num_states, num_actions, grid_size)

    mdp.phi = phi
    mdp.dim = dim  # critical! set dimension properly for agents to use
    return mdp
